(:~
    ...
    ODD: /db/apps/dodis/resources/odd/songs.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/songs/web";
...
declare %private function model:template-pb($config as map(*), $node as node()*, $params as map(*)) {
    <t xmlns=""><span class="page-break">Page {$config?apply-children($config, $node, $params?@n)}</span></t>/*
};
    ...
    $input !         (
            ...
            return
                    ...
                    case element(l) return
                        if (@rend='indent') then
                            html:block($config, ., ("tei-l1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            html:block($config, ., ("tei-l2", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                    ...
                    case element(pb) return
                        ...
                        return
                                                html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-pb", css:map-rend-to-class(.)), $content)                        => model:map($node, $trackIds)
                    ...
                    case element(lg) return
                        if (@type='stanza') then
                            html:block($config, ., ("tei-lg1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            html:block($config, ., ("tei-lg2", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                    ...
                    case element(head) return
                        if (parent::div1) then
                            html:heading($config, ., ("tei-head1", css:map-rend-to-class(.)), ., 2)                            => model:map($node, $trackIds)
                        else
                            if (parent::div2) then
                                html:heading($config, ., ("tei-head2", css:map-rend-to-class(.)), ., 3)                                => model:map($node, $trackIds)
                            else
                                html:heading($config, ., ("tei-head3", css:map-rend-to-class(.)), ., ())                                => model:map($node, $trackIds)
                    ...
                    case element(body) return
                        html:block($config, ., ("tei-body", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(text) return
                        html:block($config, ., ("tei-text", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(div1) return
                        if (@type='book') then
                            (
                                html:section($config, ., ("tei-div11", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            )

                        else
                            html:section($config, ., ("tei-div12", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                    case element(div2) return
                        if (@type='poem') then
                            (
                                html:section($config, ., ("tei-div21", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            )

                        else
                            html:section($config, ., ("tei-div22", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                    ...
                    case text() | xs:anyAtomicType return
                        html:text($config, ., ("tei--text", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    default return 
                        $config?apply($config, ./node())
