...
module namespace model="http://www.tei-c.org/pm/models/songs/latex";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(l) return
                        if (@rend='indent') then
                            latex:block($config, ., ("tei-l1", css:map-rend-to-class(.)), .)
                        else
                            latex:block($config, ., ("tei-l2", css:map-rend-to-class(.)), .)
                    ...
                    case element(pb) return
                        ...
                        return
                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-pb", css:map-rend-to-class(.)), $content)
                    ...
                    case element(lg) return
                        if (@type='stanza') then
                            latex:block($config, ., ("tei-lg1", css:map-rend-to-class(.)), .)
                        else
                            latex:block($config, ., ("tei-lg2", css:map-rend-to-class(.)), .)
                    ...
                    case element(head) return
                        if (parent::div1) then
                            latex:heading($config, ., ("tei-head1", css:map-rend-to-class(.)), ., 2)
                        else
                            if (parent::div2) then
                                latex:heading($config, ., ("tei-head2", css:map-rend-to-class(.)), ., 3)
                            else
                                latex:heading($config, ., ("tei-head3", css:map-rend-to-class(.)), ., ())
                    ...
                    case element(body) return
                        latex:block($config, ., ("tei-body", css:map-rend-to-class(.)), .)
                    ...
                    case element(text) return
                        latex:block($config, ., ("tei-text", css:map-rend-to-class(.)), .)
                    ...
                    case element(div1) return
                        if (@type='book') then
                            (
                                latex:section($config, ., ("tei-div11", css:map-rend-to-class(.)), .)
                            )

                        else
                            latex:section($config, ., ("tei-div12", css:map-rend-to-class(.)), .)
                    case element(div2) return
                        if (@type='poem') then
                            (
                                latex:section($config, ., ("tei-div21", css:map-rend-to-class(.)), .)
                            )

                        else
                            latex:section($config, ., ("tei-div22", css:map-rend-to-class(.)), .)
                    case element() return
                    ...
                    case text() | xs:anyAtomicType return
                        latex:text($config, ., ("tei--text", css:map-rend-to-class(.)), .)
                    default return 
                        $config?apply($config, ./node())
