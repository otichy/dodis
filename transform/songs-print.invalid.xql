...
module namespace model="http://www.tei-c.org/pm/models/songs/print";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(l) return
                        if (@rend='indent') then
                            html:block($config, ., ("tei-l1", css:map-rend-to-class(.)), .)
                        else
                            html:block($config, ., ("tei-l2", css:map-rend-to-class(.)), .)
                    ...
                    case element(pb) return
                        ...
                        return
                                                html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-pb", css:map-rend-to-class(.)), $content)
                    ...
                    case element(lg) return
                        if (@type='stanza') then
                            html:block($config, ., ("tei-lg1", css:map-rend-to-class(.)), .)
                        else
                            html:block($config, ., ("tei-lg2", css:map-rend-to-class(.)), .)
                    ...
                    case element(head) return
                        if (parent::div1) then
                            html:heading($config, ., ("tei-head1", css:map-rend-to-class(.)), ., 2)
                        else
                            if (parent::div2) then
                                html:heading($config, ., ("tei-head2", css:map-rend-to-class(.)), ., 3)
                            else
                                html:heading($config, ., ("tei-head3", css:map-rend-to-class(.)), ., ())
                    ...
                    case element(body) return
                        html:block($config, ., ("tei-body", css:map-rend-to-class(.)), .)
                    ...
                    case element(text) return
                        html:block($config, ., ("tei-text", css:map-rend-to-class(.)), .)
                    ...
                    case element(div1) return
                        if (@type='book') then
                            (
                                html:section($config, ., ("tei-div11", css:map-rend-to-class(.)), .)
                            )

                        else
                            html:section($config, ., ("tei-div12", css:map-rend-to-class(.)), .)
                    case element(div2) return
                        if (@type='poem') then
                            (
                                html:section($config, ., ("tei-div21", css:map-rend-to-class(.)), .)
                            )

                        else
                            html:section($config, ., ("tei-div22", css:map-rend-to-class(.)), .)
                    ...
                    case text() | xs:anyAtomicType return
                        html:text($config, ., ("tei--text", css:map-rend-to-class(.)), .)
                    default return 
                        $config?apply($config, ./node())
